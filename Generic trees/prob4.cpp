// count the no of nodes in the tree and  also the sum of all the nodes in the tree 
// we just call his childhren and solve these type of questions very easily by recursion ;
...
int sum_allnodes(treenode<int>*root){
    int sum =root->data ;
    for(int i =0 ;i<root->children.size();i++){
        sum += sum_allnodes(root->children[i]) ;
    }
    return sum ;
}
...
int main(){
    ...
    printnode(root) ;
    int ans =num_nodes(root) ;
    int sum = sum_allnodes(root);
    cout<< " number of nodes in the tree :" <<ans<<endl ;
    cout<<" sum of all the nodes int the tree :"<<sum<<endl;
}